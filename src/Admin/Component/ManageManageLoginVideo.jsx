...
const ManageManageLoginVideo = () => {
    ...
    const fetchVideos = async () => {
        try {
            setLoading(true);
            const res = await api.get('/manage-login-video');
            setVideos(res.data.reverse());
        } catch (err) {
            console.error(err);
            toast.error('Failed to load manage login videos');
        } finally {
            setLoading(false);
        }
    };
    ...
    const saveEdit = async (id) => {
        ...
        try {
            const res = await api.patch(`/manage-login-video/${id}`, payload);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const res = await api.delete(`/manage-login-video/${id}`);
            ...
            >
                <div className="mb-8">
                    <h2 className="text-2xl md:text-3xl font-bold text-gray-800">
                        Manage "Manage Login" Videos
                    </h2>
                    <p className="text-gray-600 mt-2">
                        Edit or delete saved video links for managing login
                    </p>
                </div>
...
export default ManageManageLoginVideo;
